...
def average_roc(rocs, fps):    
    ...
    # fps = filter(lambda f: f < 1, fps) ##///////////////Original///////////////////////
    fps = list(filter(lambda f: f < 1, fps)) ##//////////////Changed////////////////////
    tps = np.zeros((len(rocs), len(fps)))
    print(len(fps), len(rocs))
    ...
    for i in range(len(rocs)):
        ...
        for j in range(len(fps)):
            # print(r[1,:], fps[j])
            # print(np.nonzero(r[1,:] > fps[j]))
            k = np.min(np.nonzero(r[1,:] > fps[j])) ## //////////////give empty some time//////Not solved
